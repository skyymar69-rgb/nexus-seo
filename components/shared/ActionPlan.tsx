  ...
  return (
    <div className="rounded-lg overflow-hidden border border-surface-200 dark:border-surface-700 mt-2">
      <div className="flex items-center justify-between px-3 py-1.5 bg-surface-100 dark:bg-surface-800 text-xs">
        <div>
          <span className="font-semibold text-foreground">{snippet.label}</span>
          <span className="text-muted-foreground ml-2">({snippet.where})</span>
        </div>
        <button onClick={copy} className="flex items-center gap-1 text-muted-foreground hover:text-surface-700 transition-colors">
    ...
    <div className={cn(
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            ...
            <span className="text-[10px] text-muted-foreground">{rec.category} | {rec.estimatedTime}</span>
          </div>
      ...
      {expanded && (
        <div className="border-t border-surface-100 dark:border-surface-800 px-4 py-4 space-y-4 bg-surface-50/50 dark:bg-surface-800/30">
          ...
          {rec.steps.length > 0 && (
            <div>
              <h4 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">Etapes a suivre</h4>
          ...
          {rec.tools.length > 0 && (
            <div>
              <h4 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">Outils recommandes</h4>
          ...
          {rec.codeSnippets.length > 0 && (
            <div>
              <h4 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">Code a implanter</h4>
  ...
  return (
    <div className="space-y-4 print:space-y-2">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-xl bg-white dark:bg-surface-900 border border-surface-200 dark:border-surface-800 px-5 py-4">
        <div>
          <h3 className="text-lg font-bold text-foreground">Plan d&apos;action SEO</h3>
        ...
        <div className="flex items-center gap-2 print:hidden">
          {onPrint && (
            <button onClick={onPrint} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border border-surface-200 dark:border-surface-700 hover:bg-surface-50 dark:hover:bg-surface-800 text-foreground transition-colors">
              <Printer className="w-3.5 h-3.5" /> Imprimer
            </button>
          )}
          {onExportMD && (
            <button onClick={onExportMD} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border border-surface-200 dark:border-surface-700 hover:bg-surface-50 dark:hover:bg-surface-800 text-foreground transition-colors">
              <FileDown className="w-3.5 h-3.5" /> Markdown
            </button>
          )}
          {onExportHTML && (
            <button onClick={onExportHTML} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border border-surface-200 dark:border-surface-700 hover:bg-surface-50 dark:hover:bg-surface-800 text-foreground transition-colors">
